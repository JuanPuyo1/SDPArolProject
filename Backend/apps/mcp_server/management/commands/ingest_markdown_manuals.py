...
from apps.mcp_server.rag_engine.collections import clear_manuals_collection, ensure_manuals_collection
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for md_path in md_files:
            ...
            self.stdout.write(f'  -> Ingesting {md_path.name} for serial [{meta.machine_serial}]...')
            ...
            self.stdout.write(
                self.style.SUCCESS(f'     Ingested {num_vectors} vectors from {md_path.name} ({elapsed:.2f}s)')
            )
        ...
        self.stdout.write('=' * 65)
        self.stdout.write(f'Successfully ingested : {len(summary)} / {len(md_files)} manuals')
        ...
        for name, serial, count, elapsed in summary:
            self.stdout.write(f' • {name:28s} | Serial: {serial:12s} | Vectors: {count:4d} | {elapsed:.2f}s')
        self.stdout.write('=' * 65)
